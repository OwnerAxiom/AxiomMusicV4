...
from AxiomMusic.utils.database import blacklist_chat, blacklisted_chats, whitelist_chat
...
async def all_chats(client, message: Message, _):
    ...
    for count, chat_id in enumerate(await blacklisted_chats(), 1):
        try:
            ...
            title = "ᴘʀɪᴠᴀᴛᴇ ᴄʜᴀᴛ"
        j = 1
        text += f"<blockquote expandable><b>✧ {count}. {title}[<code>{chat_id}</code>] </b></blockquote>\n"
    if j == 0:

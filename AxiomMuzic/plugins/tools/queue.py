...
from AxiomMusic.utils import AxiommBin, get_channeplayCB, seconds_to_min
from AxiomMusic.utils.database import get_cmode, is_active_chat, is_music_playing
from AxiomMusic.utils.decorators.language import language, languageCB
from AxiomMusic.utils.inline import queue_back_markup, queue_markup
...
async def get_queue(client, message: Message, _):
    ...
    basic[videoid] = True
    mystic = await message.reply_photo(IMAGE, has_spoiler=True, caption=cap, reply_markup=upl)
    if DUR != "Unknown":
        try:
...
async def queued_tracks(client, CallbackQuery: CallbackQuery, _):
    ...
    med = InputMediaPhoto(
        media="https://files.catbox.moe/m4fx24.jpg",
        has_spoiler=True,
        caption=_["queue_1"],
    )
    ...
    for x in got:
        j += 1
        if j == 1:
            msg += f'<blockquote expandable><b>✧ Streaming :\n\n✧ Title : {x["title"]}\n✧ Duration : {x["dur"]}\n✧ By : {x["by"]} </b></blockquote>\n\n'
        elif j == 2:
            msg += f'<blockquote expandable><b>✧ Queued :\n\n✧ Title : {x["title"]}\n✧ Duration : {x["dur"]}\n✧ By : {x["by"]} </b></blockquote>\n\n'
        else:
            msg += f'<blockquote expandable><b>✧ Title : {x["title"]}\n✧ Duration : {x["dur"]}\n✧ By : {x["by"]} </b></blockquote>\n\n'
    if "Queued" in msg:
        if len(msg) < 700:
...
async def queue_back(client, CallbackQuery: CallbackQuery, _):
    ...
    med = InputMediaPhoto(media=IMAGE, has_spoiler=True, caption=cap)

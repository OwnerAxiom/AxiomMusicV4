...
def _truncate(draw, text, font, max_w):
    if _get_text_width(draw, text, font) <= max_w:
        return text
    while text and _get_text_width(draw, text + "…", font) > max_w:
        text = text[:-1]
    return text + "…"
...
async def get_thumb(videoid: str, user_name: str = "AxiomUser") -> str:
    ...
    if thumb_url:
        try:
            ...
            song_img = Image.open(cache).resize((110, 110), Image.LANCZOS).convert("RGBA")
        except Exception as e:
    ...
    card_draw.rounded_rectangle([0, 0, CARD_W-1, CARD_H-1], radius=CARD_RADIUS, fill=card_bg)
    
    # Top light effect
    for y in range(0, 150):
        alpha = int(60 * (1 - y/150))
        card_draw.line([(10, y), (CARD_W-10, y)], fill=(255, 255, 255, alpha))
    
    # Border
    card_draw.rounded_rectangle([2, 2, CARD_W-3, CARD_H-3], radius=CARD_RADIUS-2, 
    ...
    title_y = 55
    card_draw.text((title_x, title_y), title, fill=WHITE, font=font_title)
    
    # Green heart
    title_width = _get_text_width(card_draw, title, font_title)
    heart_x = title_x + title_width + 12
    card_draw.text((heart_x, title_y + 2), "💚", fill=GREEN_PRIMARY, font=font_heart)

    # Channel and views
    subtitle_y = title_y + 52
    card_draw.text((title_x, subtitle_y), channel, fill=(230, 230, 230), font=font_subtitle)
    channel_width = _get_text_width(card_draw, channel, font_subtitle)
    views_x = title_x + channel_width + 30
    card_draw.text((views_x, subtitle_y), views, fill=(200, 200, 200), font=font_subtitle)
    ...
    wave_w = CARD_W - 90
    wave_h = 55
    _draw_waveform(card_draw, wave_x, wave_y, wave_w, wave_h, GREEN_WAVEFORM, progress)

    # Progress bar
    prog_y = 280
    prog_x = 45
    prog_w = CARD_W - 90
    _draw_progress_bar(card_draw, prog_x, prog_y, prog_w, progress, GREEN_PRIMARY, GREEN_LIGHT)
    
    card_draw.text((prog_x, prog_y + 18), current_time, fill=WHITE, font=font_time)
    dur_width = _get_text_width(card_draw, duration, font_time)
    card_draw.text((prog_x + prog_w - dur_width, prog_y + 18), duration, fill=WHITE, font=font_time)
    ...
    shuffle_x = 90
    _draw_shuffle_icon(card_draw, shuffle_x, ctrl_y, 50, GREEN_PRIMARY)
    
    prev_x = center_x - 170
    _draw_skip_triangle(card_draw, prev_x, ctrl_y, 45, BLACK, direction=-1)
    
    play_x = center_x
    play_y = ctrl_y + 5
    play_r = 48
    card_draw.ellipse([play_x-play_r-4, play_y-play_r-4, play_x+play_r+4, play_y+play_r+4], fill=(76, 175, 80, 80))
    card_draw.ellipse([play_x-play_r, play_y-play_r, play_x+play_r, play_y+play_r], 
                     fill=GREEN_DARK, outline=GREEN_LIGHT, width=2)
    card_draw.ellipse([play_x-play_r+8, play_y-play_r+8, play_x+play_r-8, play_y-10], fill=(100, 200, 100, 60))
    _draw_play_triangle(card_draw, play_x+3, play_y, 30, WHITE)
    
    next_x = center_x + 170
    _draw_skip_triangle(card_draw, next_x, ctrl_y, 45, BLACK, direction=1)
    
    repeat_x = CARD_W - 90
    _draw_repeat_icon(card_draw, repeat_x, ctrl_y, 50, GREEN_PRIMARY)
    ...
    for i, (icon, label) in enumerate(buttons):
        ...
        card_draw.rounded_rectangle([bx, btn_y, bx+btn_w, btn_y+btn_h], radius=btn_r, fill=btn_bg)
        card_draw.text((bx+20, btn_y+10), icon, fill=WHITE, font=font_btn_icon)
        card_draw.text((bx+55, btn_y+13), label, fill=WHITE, font=font_btn_label)

    # Volume bar
    vol_y = 545
    vol_x = 70
    vol_w = CARD_W - 140
    _draw_volume_bar(card_draw, vol_x, vol_y, vol_w, 0.55, GREEN_PRIMARY)
    
    vol_icon_font = _get_font(FONT_NORMAL, 24)
    card_draw.text((25, vol_y-12), "🔈", fill=WHITE, font=vol_icon_font)
    card_draw.text((CARD_W-50, vol_y-12), "🔊", fill=WHITE, font=vol_icon_font)
